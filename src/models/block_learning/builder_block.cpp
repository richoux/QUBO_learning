...
void BuilderQUBO::declare_constraints()
{
	...
	constraints.emplace_back( make_shared< ParameterManagement >( std::vector<int>{8}, _parameter ) );
}

void BuilderQUBO::declare_objective()
{
	objective = std::make_shared<ObjectiveBlock>( variables, _training_data, _size_training_set, _candidate_length, _domain_size, _starting_value, _error_vector, _parameter );
}
