		...
		for( size_t col = row ; col < matrix_side ; ++col )
		{
			...
			if( col == row ) //diagonal
			{
				if( variables[0] == 2 ) // -1-diagonal pattern
					...
					{
						if( _parameter == std::numeric_limits<int>::max() )
							param = 1;
						else
							param = _parameter;
							
						Q( row, col ) += -( 2 * param - ( row_domain + _starting_value ) ) * ( row_domain + _starting_value );
					}
			}
			else // non-diagonal
			{
				...
				else // full-block
				{
					...
					if( variables[9] == 1 ) // attract pattern
					{
						...
						Q( row, col ) += std::max( 0, std::abs( col_domain - row_domain ) - ( static_cast<int>( _domain_size ) - 1 - param ) );
					}

					if( variables[10] == 1 ) // swapped values pattern
					{
						if( ( row / _domain_size ) == col_domain && ( col / _domain_size ) == row_domain )
							Q( row, col ) += -1;
						else
							Q( row, col ) += 1;
					}
