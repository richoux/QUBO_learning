...
int main( string[] args )
{
	...
	foreach( line; file_pattern_vectors.byLine() )
	{
		...
		else
		{
			...
			string command = "../../bin/learn_q_opt -e 1 -f " ~ filename_test ~ " -c solution_tmp --benchmark 2> /dev/null";
			writeln( "Executing ../../bin/learn_q_opt -e 1 -f " ~ filename_test ~ " -c " ~ vector ~ " --benchmark 2> /dev/null" );
